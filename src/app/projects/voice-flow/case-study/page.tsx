...
import { voiceFlowCaseStudy as cs } from "@/data/case-studies/voice-flow";

export default function VoiceFlowCaseStudyPage() {
  return (
    <div className="min-h-screen bg-zinc-50">
      ...
      <article className="mx-auto max-w-3xl px-6 py-12">
        <p className="text-sm font-medium tracking-wide text-indigo-600 uppercase">
          Case study · Real-Time Voice & Translation
        </p>
        ...
        <div className="mt-10 rounded-xl border border-zinc-200 bg-white p-6">
          ...
          <div className="flex flex-wrap items-center justify-center gap-2 text-sm font-medium text-zinc-700">
            <FlowBox label="Speaker ends" />
            <Arrow />
            <FlowBox label="STT Hop" highlight />
            <Arrow />
            <FlowBox label="Translation Hop" highlight />
            <Arrow />
            <FlowBox label="TTS Hop" highlight />
            <Arrow />
            <FlowBox label="Audio output" />
          </div>
          <p className="mt-4 text-center text-xs text-zinc-500">
            End-to-end latency = STT + Translation + TTS + pipeline overhead
          </p>
        </div>
        ...
        <Section icon={<Mic className="h-5 w-5" />} title="Research">
        ...
        <Section icon={<BarChart3 className="h-5 w-5" />} title="Results">
          ...
          <h3 className="mt-8 font-semibold text-zinc-900">Configuration matrix</h3>
          <div className="mt-4 overflow-hidden rounded-lg border border-zinc-200">
            <table className="w-full text-sm">
              <thead className="bg-zinc-50">
                <tr>
                  <th className="px-4 py-2 text-left font-medium text-zinc-600">Config</th>
                  <th className="px-4 py-2 text-left font-medium text-zinc-600">E2E</th>
                  <th className="px-4 py-2 text-left font-medium text-zinc-600">BLEU</th>
                  <th className="px-4 py-2 text-left font-medium text-zinc-600">Verdict</th>
                </tr>
              </thead>
              <tbody>
                {cs.results.sampleRoutes.map((r) => (
                  <tr key={r.config} className="border-t border-zinc-100">
                    <td className="px-4 py-3 text-zinc-800">{r.config}</td>
                    <td className="px-4 py-3 text-zinc-600">{r.e2e}</td>
                    <td className="px-4 py-3 text-zinc-600">{r.bleu}</td>
                    <td className="px-4 py-3 text-zinc-600">{r.verdict}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <Section icon={<ArrowUpRight className="h-5 w-5" />} title="Tradeoffs">
        ...
        <div className="mt-16 rounded-xl bg-indigo-600 p-8 text-center text-white">
          <h2 className="text-xl font-bold">Tune your pipeline</h2>
          <p className="mt-2 text-indigo-100">
            Adjust chunk size and model tier to explore the latency vs. quality frontier.
          </p>
          <Link
            href={cs.links.liveDemo}
