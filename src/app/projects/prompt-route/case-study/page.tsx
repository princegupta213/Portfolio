...
import { promptRouteCaseStudy as cs } from "@/data/case-studies/prompt-route";

export default function PromptRouteCaseStudyPage() {
  return (
    <div className="min-h-screen bg-zinc-50">
      ...
      <article className="mx-auto max-w-3xl px-6 py-12">
        <p className="text-sm font-medium tracking-wide text-indigo-600 uppercase">
          Case study · Platform & Core Infrastructure
        </p>
        ...
        <div className="mt-10 rounded-xl border border-zinc-200 bg-white p-6">
          ...
          <div className="flex flex-wrap items-center justify-center gap-2 text-sm font-medium text-zinc-700">
            <FlowBox label="App Request" />
            <Arrow />
            <FlowBox label="Classifier" highlight />
            <Arrow />
            <FlowBox label="Routing Table" highlight />
            <Arrow />
            <FlowBox label="Primary Model" />
            <Arrow />
            <FlowBox label="Response" />
          </div>
          <p className="mt-4 text-center text-xs text-zinc-500">
            On HTTP 429 / timeout → fallback model · circuit breaker flags degraded provider
          </p>
        </div>
        ...
        <Section icon={<Server className="h-5 w-5" />} title="Research">
        ...
        <Section icon={<BarChart3 className="h-5 w-5" />} title="Results">
          ...
          <h3 className="mt-8 font-semibold text-zinc-900">Sample route decisions</h3>
          <div className="mt-4 overflow-hidden rounded-lg border border-zinc-200">
            <table className="w-full text-sm">
              <thead className="bg-zinc-50">
                <tr>
                  <th className="px-4 py-2 text-left font-medium text-zinc-600">Task</th>
                  <th className="px-4 py-2 text-left font-medium text-zinc-600">Primary</th>
                  <th className="px-4 py-2 text-left font-medium text-zinc-600">Fallback</th>
                  <th className="px-4 py-2 text-left font-medium text-zinc-600">Outcome</th>
                </tr>
              </thead>
              <tbody>
                {cs.results.sampleRoutes.map((r) => (
                  <tr key={r.task} className="border-t border-zinc-100">
                    <td className="px-4 py-3 text-zinc-800">{r.task}</td>
                    <td className="px-4 py-3 text-zinc-600">{r.primary}</td>
                    <td className="px-4 py-3 text-zinc-600">{r.fallback}</td>
                    <td className="px-4 py-3">
                      <span
                        className={`rounded-full px-2 py-0.5 text-xs font-medium ${
                          r.outcome.startsWith("Fallback")
        ...
        <Section icon={<Route className="h-5 w-5" />} title="Tradeoffs">
        ...
        <div className="mt-16 rounded-xl bg-indigo-600 p-8 text-center text-white">
          <h2 className="text-xl font-bold">Route your first batch</h2>
          <p className="mt-2 text-indigo-100">
            Run 8 mixed prompts through the router and see cost savings, failover, and latency metrics.
          </p>
          <Link
            href={cs.links.liveDemo}
