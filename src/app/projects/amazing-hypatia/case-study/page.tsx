...
import { amazingHypatiaCaseStudy as cs } from "@/data/case-studies/amazing-hypatia";

export default function AmazingHypatiaCaseStudyPage() {
  return (
    <div className="min-h-screen bg-zinc-50">
      <header className="border-b border-zinc-200 bg-white">
        <div className="mx-auto flex max-w-3xl items-center justify-between px-6 py-4">
          ...
          <Link
            ...
            target="_blank"
            className="inline-flex items-center gap-1.5 text-sm font-medium text-indigo-650 hover:text-indigo-850"
          >
      ...
      <article className="mx-auto max-w-3xl px-6 py-12">
        <p className="text-sm font-medium tracking-wide text-indigo-600 uppercase">
          Case study · Agentic AI · Product Launch Employee
        </p>
        ...
        <div className="mt-10 rounded-xl border border-zinc-200 bg-white p-6">
          <p className="mb-4 text-xs font-semibold uppercase tracking-wide text-indigo-600">
            Launch employee coordination flow
          </p>
          <div className="flex flex-wrap items-center justify-center gap-2 text-sm font-medium text-zinc-700">
            <FlowBox label="Folder Onboarding" />
            <Arrow />
            <FlowBox label="Proactive check logs" highlight />
            <Arrow />
            <FlowBox label="Consistency Engine" highlight />
            <Arrow />
            <FlowBox label="Time Travel checks" />
          </div>
          <p className="mt-4 text-center text-xs text-zinc-500">
            Enterprise presets · dynamic checklists · parameter synchronization · metrics logs
          </p>
        </div>
        ...
        <Section icon={<Sparkles className="h-5 w-5" />} title="Solution">
        ...
        <Section icon={<BarChart3 className="h-5 w-5" />} title="Results">
          ...
          {"sampleOutcomes" in cs.results && cs.results.sampleOutcomes && (
            <>
              <h3 className="mt-8 font-semibold text-zinc-900">Post-Launch Milestones</h3>
              <div className="mt-4 overflow-hidden rounded-lg border border-zinc-200">
                <table className="w-full text-sm">
                  <thead className="bg-zinc-50">
                    <tr>
                      <th className="px-4 py-2 text-left font-medium text-zinc-600">Milestone</th>
                      <th className="px-4 py-2 text-left font-medium text-zinc-600">Simulated Anomaly</th>
                      <th className="px-4 py-2 text-left font-medium text-zinc-600">Resolution Status</th>
                      <th className="px-4 py-2 text-left font-medium text-zinc-600">Errors</th>
                    </tr>
                  </thead>
                  <tbody>
                    {cs.results.sampleOutcomes.map((r) => (
                      <tr key={r.scenario} className="border-t border-zinc-100">
                        <td className="px-4 py-3 text-zinc-800 font-medium">{r.scenario}</td>
                        <td className="px-4 py-3 text-zinc-600">{r.batch}</td>
                        <td className="px-4 py-3 text-indigo-700 font-bold">{r.compliance}</td>
                        <td className="px-4 py-3 text-zinc-600">{r.duplicates}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
        ...
        <div className="mt-16 rounded-xl bg-indigo-600 p-8 text-center text-white">
          <h2 className="text-xl font-bold">Try the AI Product Launch Employee</h2>
          <p className="mt-2 text-indigo-100">
            Experience real-time parameter synchronization, time travel logs, and multi-document onboarding folders.
          </p>
          <Link
            href={cs.links.liveDemo}
            target="_blank"
            className="mt-6 inline-flex items-center gap-2 rounded-lg bg-white px-6 py-3 text-sm font-semibold text-indigo-650 hover:bg-indigo-50"
          >
  ...
  return (
    <div
      className={`rounded-lg px-4 py-2 ${
        highlight ? "bg-indigo-55 text-indigo-800 ring-1 ring-indigo-200" : "bg-zinc-100 text-zinc-700"
      }`}
    >
      {label}
    </div>
